    ...
    function recaptcha_styles(){ ?>
		<script>
			...
			if(recpErr){
				...
				recpErr = parseInt(sessionStorage.getItem("recapthcaError"));
			}

			// Global word choice
			var recpRandomWords = ['allegation','conference','domicile','emancipated','garnishment','incarceration','juvenile','magistrate','lawyers','attorneys','defendant','plaintiff','deposition','notarize','vacate','stipulation','summons','witness','prosecutor','investigatory','foreclosure'];
			...
			function validate(){
				...
				if(interval < 2){
					...
					if(step6 !== ''){
						...
						let check2 = document.querySelector('#'+dataID+' .answer').value;
			...
			function holdPopUp(customMessage){
				...
				const popupContainer = document.createElement("div");
					...
					if(customMessage){
						...
						popupContainer.innerHTML = "<div class='wrapper'><p class'sorry'>Sorry, too many failed attempts. Please try again later.</p></div>";	
					}
        ...
        <style>
          ...
          .recaptcha-wrapper > div{width: 100%; position: fixed; top: 50%; transform: translateY(-50%); background-color: #0d4d94; z-index: 1000000000; left: 0; right: 0; margin: 0 auto; max-width: 700px; padding: 25px;}
          .recaptcha-wrapper .title{font-weight: 600; font-size: 20px; padding: 0;margin: 0; color:var(--white);} 
          .recaptcha-wrapper .explain{padding: 10px 0;margin: 0; color:var(--white);}
          .recaptcha-wrapper ul{padding: 0 !important;margin: 0 0 0 35px !important;}
          .recaptcha-wrapper ul li{list-style-type: disc !important; color:var(--white); margin: 0 !important;}
          .recaptcha-wrapper .attention{padding: 15px 0 0;margin: 0;  font-weight: 600; color:var(--white);}
          ...
          .recaptcha-wrapper .answer{width: calc(100% - 150px) !important; display: block; width: 100%; border: none; padding-left: 15px !important; font-family: var(--text-font);}
          .recaptcha-wrapper .button{width: 150px;}
          /*message*/
          .recaptcha-wrapper .message{padding: 0;margin: 0; height: 100%; max-height: 0; transition: max-height .25s ease-in;}
          	...
          	/*exit*/
          	.recaptcha-wrapper .exit{position: absolute; font-weight: 900; font-size: 24px; top: 0px; right: -25px; color:var(--white); margin: 0; line-height: normal; padding: 0;}
          		.recaptcha-wrapper .exit:hover{cursor: pointer; color:var(--lt-blue);}
          	/*button*/
          	.gform_footer.active{display: block;}
          	.gform_footer{display: none;}
          	.gform_footer.active{display: block;}
          	.gform_body.gform-body .recaptcha-button.button{margin: 0 !important;}
          	.gform_body.gform-body .recaptcha-button.button.hide{display: none;}
          	/*notice*/
          	form .notice{margin: 0; padding: 10px 0 !important;}
          ...
          @media all and (max-width: 480px){
          	.recaptcha-wrapper .answer{width: 100% !important; padding: 10px;}
